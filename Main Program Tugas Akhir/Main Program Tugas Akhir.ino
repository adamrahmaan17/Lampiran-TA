...
float phClass, tempClass, tdsClass;  
...
void tampilUlasan(){
    ...
    lcd.setCursor(0,0); lcd.print(outputClassification);
    String xph = String(phClass,2), xcels = String(tempClass,1), xtds = String(tdsClass,0);
    
    String mitem2 = String("ph   : " + xph + "%");
    String mitem3 = String("Suhu : " + xcels + "%");
    String mitem4 = String("TDS  : " + xtds + "%");
    lcd.setCursor(0,1); lcd.print(mitem2);
    lcd.setCursor(0,2); lcd.print(mitem3);
    lcd.setCursor(0,3); lcd.print(mitem4);
...
void waterClassification (float avgPh, float avgTemp, float avgTds){
  ...
  if (avgPh <= 6.3 ){
    ...
    phClass = 16.8;
    recomendationAction = "beri kapur(sedang)";
  }
  else if (avgPh <= 6.9 ){
    phClass = 33.4;
    ...
    phClass = 50;
    recomendationAction = "pH sudah optimal";
  }
  else if (avgPh <= 8.5 ){
    phClass = 33.4;
    recomendationAction = "beri tawas(sedikit)";
  }
  else if (avgPh <= 8.9 ){
    phClass = 16.8;
    recomendationAction = "Beri Tawas(banyak)";
  }
  else {
    phClass = 0;
    recomendationAction ="kuras/ganti air";
  }

  //Klasifikasi Nilai Suhu
  ...
  if (avgTds > 300 && avgTds <= 600){
    tdsClass = 30;
    recomendationActiontds ="TDS sudah optimal";
  }else if (avgTds > 600 && avgTds <= 1000){
    tdsClass = 20;
    recomendationActiontds ="filter/ganti air";
  }else if (avgTds > 1000 && avgTds <= 1200){
    tdsClass = 10;
    recomendationActiontds ="filter(deionisasi)";
  }else{
    tdsClass = 0;
   recomendationActiontds ="kuras air tambak";
  }
}

String stringClassification(){
  //Klasifikasi Akhir
  float totalClasification = phClass + tempClass + tdsClass;
  if(totalClasification <= 25){
    return "Sangat Buruk";
  }else if(totalClasification <= 51){
    return "Buruk";
  }else if(totalClasification <= 77){
    return "Kurang Baik";
  }else{
    return "Baik";
  }
